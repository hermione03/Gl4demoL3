...
static void init( void ) {
    ...
    _pId = gl4duCreateProgram( "<vs>shaders/fractal.vs", "<fs>shaders/fractal.fs", NULL );
...
static void draw( void ) {
    ...
    // int wx, wy;
    // SDL_GetWindowSize( gl4duwGetWindow(), &wx, &wy );
    // glUniform2f( glGetUniformLocation( _pId, "u_resolution" ), wx, wy );
